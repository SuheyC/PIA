<template>
    <div class="contenedor-agregar-admin">
      <div class="formulario-admin">
        ...
        <input type="text" v-model="nombreUsuario" placeholder="Nombre del estudiante" />
        ...
        <input type="text" v-model="numeroEstudiante" placeholder="Número de estudiante" />
        <p v-if="errores.numeroEstudiante" class="mensaje-error">{{ errores.numeroEstudiante }}</p>
  ...
  export default {
    ...
    created() {
      const estudiante = JSON.parse(localStorage.getItem('estudianteEditar'));
      if (estudiante) {
        this.nombreUsuario = estudiante.nombre || '';
        this.correoElectronico = estudiante.correo || '';
        this.numeroEstudiante = estudiante.matricula || '';  // Se usa 'matricula' en lugar de 'numeroEstudiante'
        if (estudiante.foto) {
          this.imagenPreview = estudiante.foto;
        }
      } 
    },
    methods: {
      ...
      guardarCambios() {
        ...
        if (!this.numeroEstudiante) {
          this.errores.numeroEstudiante = 'El número de estudiante es obligatorio.';
        } else if (!this.contieneNumero(this.numeroEstudiante)) {
          this.errores.numeroEstudiante = 'Debe contener al menos un número.';
        }
        ...
        const estudianteEditado = {
          ...
          matricula: this.numeroEstudiante,  // Se usa 'matricula' en lugar de 'numeroEstudiante'
          foto: this.imagenPreview,
        };
  
        console.log('Estudiante editado:', estudianteEditado);
        alert('Cambios guardados correctamente.');
        localStorage.setItem('estudianteEditar', JSON.stringify(estudianteEditado));  // Guarda los cambios en localStorage
        this.$router.push('/admin/usuariosEstudiantes');
      },
      cancelar() {
        const hayDatos = this.nombreUsuario || this.correoElectronico || this.numeroEstudiante || this.imagenPreview || this.nuevaContrasena;
