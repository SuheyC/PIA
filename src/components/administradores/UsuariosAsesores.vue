<template>
    <div class="contenedor-principal">
      ...
      <main class="contenedor">
        <div class="cabecera-usuarios">
          <h1 class="titulo-usuarios">Usuarios Asesores</h1>
         <!--  <button @click="agregarAsesor" class="boton-agregar">
            + Agregar Asesor
          </button> -->
        </div>
  
        <!-- Contenedor centrado de usuarios -->
        <div class="contenedor-usuarios">
          <div v-for="asesor in paginatedAsesores" :key="asesor.id" class="tarjeta-usuario">
            <div class="contenido-tarjeta">
              <div class="imagen-usuario">
                <img v-if="asesor.foto" :src="asesor.foto" class="imagen-tarjeta" />
                <i v-else class="fa fa-user-circle fa-4x text-blue-500"></i>
              </div>
              <h2 class="nombre-usuario">{{ asesor.nombre }}</h2>
              <p class="correo-usuario">{{ asesor.correo }}</p>
              <p class="numero-empleado">N.º de empleado: {{ asesor.numeroEmpleado }}</p>
            </div>
            <div class="acciones-usuario">
              <button @click="editarAsesor(asesor.id)" class="boton-editar">Editar</button>
              <button @click="eliminarAsesor(asesor.id)" class="boton-eliminar">Eliminar</button>
            </div>
          </div>
        </div>
  
        <p v-if="paginatedAsesores.length === 0" class="mensaje-vacio">No hay asesores registrados.</p>
  ...
  export default {
    ...
    data() {
      return {
        asesores: [
          { id: 1, nombre: 'Laura Jiménez', correo: 'laura.jimenez@ite.edu.mx', numeroEmpleado: 'B20123', foto: '' },
          { id: 2, nombre: 'José Ramírez', correo: 'jose.ramirez@ite.edu.mx', numeroEmpleado: 'B20124', foto: '' },
          { id: 3, nombre: 'Marta Ruiz', correo: 'marta.ruiz@ite.edu.mx', numeroEmpleado: 'B20125', foto: '' },
          { id: 4, nombre: 'Ricardo Gómez', correo: 'ricardo.gomez@ite.edu.mx', numeroEmpleado: 'B20126', foto: '' },
          { id: 5, nombre: 'Carlos martinez', correo: 'carlos@ite.edu.mx', numeroEmpleado: '33445566', foto: '' }
        ],
        menuVisible: false,
    ...
    computed: {
      totalPaginas() {
        return Math.ceil(this.asesores.length / this.asesoresPorPagina);
      },
      paginatedAsesores() {
        const inicio = (this.paginaActual - 1) * this.asesoresPorPagina;
        const fin = inicio + this.asesoresPorPagina;
        return this.asesores.slice(inicio, fin);
      }
    },
    methods: {
      toggleMenu() {
      ...
      editarAsesor(id) {
      const asesor = this.asesores.find(a => a.id === id);
      localStorage.setItem('asesorEditar', JSON.stringify(asesor)); 
      this.$router.push('/admin/EditarAsesores');
      },

      eliminarAsesor(id) {
        const asesor = this.asesores.find(a => a.id === id);
        if (confirm(`¿Seguro que deseas eliminar a ${asesor.nombre}?`)) {
          this.asesores = this.asesores.filter(a => a.id !== id);
        }
      },
      agregarAsesor() {
        this.$router.push('/admin/AgregarAsesor');
      },
      editarPerfil() {
