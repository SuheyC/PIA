<template>
    <div class="inicio-estudiante">
      ...
      <!-- Contenido de Asesorías Impartidas -->
...
export default {
  ...
  methods: {
    ...
    calificarAsesoria(id) {
      this.$router.push({ name: 'CalificarAsesoria', params: { id } });
    },
    async cargarAsesorias() {
      try {
        const response = await axios.get('http://localhost:3000/asesoriasImpartidasEstudiante');
